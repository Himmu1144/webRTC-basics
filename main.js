...
let createPeerConnection = async (sdpType) => {
    ...
    // adding localstream to our peer connection

    localStream.getTracks().forEach((track) => {
        peerConnection.addTrack(track, localStream)
    })

    // an event listener which listens when the peer adds its video to the peerconnection
...
let createAnswer = async () => {
    ...
    if (!offer) return alert('first retrieve the offer!')
